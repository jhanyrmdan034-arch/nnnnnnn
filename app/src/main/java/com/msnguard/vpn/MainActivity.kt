...
class MainActivity : Activity() {
    ...
    private lateinit var tapPromptText: TextView
    ...
    private val sessionTicker = object : Runnable {
        override fun run() {
            ...
            val formatted = formatUptime(android.os.SystemClock.elapsedRealtime() - sessionStartedAt)
            orbitDial.timerText = formatted
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timerDisplay = TextView(this).apply {
            ...
            typeface = Typefaces.bold(this@MainActivity)
            gravity = Gravity.CENTER
            letterSpacing = 0.05f
        }
        tapPromptText = TextView(this).apply {
            text = Strings.t("Tap to connect")
            textSize = 14f
    ...
    private fun createHeader(): LinearLayout = LinearLayout(this).apply {
        ...
        // Shield check mark icon matching image-1.jpg
        ...
        statusLed.layoutParams = LinearLayout.LayoutParams(dp(8), dp(8)).apply {
            rightMargin = dp(8)
        }
        addView(statusLed, statusLed.layoutParams)
        addView(label(Strings.t("MSN-GUARD"), 14.5f, INK, TypefaceStyle.MEDIUM).apply {
        ...
        // Language pill selector next to gear icon: "🌐 FA"
        val langCode = when (AppLanguage.current()) {
            "fa" -> "FA"
            "zh" -> "ZH"
            else -> "EN"
        }
        val langBadge = LinearLayout(this@MainActivity).apply {
            ...
            background = Sculpt.sculptedRipple(
                ...
                accent = Sculpt.withAlpha(palette.ink, 0.12f),
            )
        ...
        addView(langBadge, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.WRAP_CONTENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply {
            rightMargin = dp(8)
    ...
    private fun fitConsoleToViewport(scroll: ScrollView, console: LinearLayout) {
        ...
        scroll.addOnLayoutChangeListener { _, _, _, _, _, _, _, _, _ ->
            ...
            val target = if (delta <= 0) 1f else (current * (dialBox - delta).toFloat() / dialBox).coerceIn(floor, 1f)
    ...
    private fun createConnectionConsole(): LinearLayout = LinearLayout(this).apply {
        ...
        // Top Block: 00:00:00 timer display and status subtitle
        addView(timerDisplay, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.WRAP_CONTENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply {
            topMargin = dp(6)
        ...
        // Flexible top spacer to center dial in screen
        ...
        // Center Container: Royal Blue Power Button and Prompt text below it
        val dialCenterContainer = LinearLayout(this@MainActivity).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER_HORIZONTAL
            clipChildren = false
            clipToPadding = false
            ...
            addView(tapPromptText, LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT,
            ).apply {
                topMargin = dp(14)
        ...
        addView(dialCenterContainer, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.WRAP_CONTENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ))

        // Flexible bottom spacer to balance top spacer
        addView(View(this@MainActivity), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            0,
            1f,
        ...
        // Server changer card: compact & chic
        ...
        val tiles = LinearLayout(this@MainActivity).apply {
            orientation = LinearLayout.HORIZONTAL
            addView(tileDown, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { rightMargin = dp(8) })
            addView(tileUp, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { leftMargin = dp(0); rightMargin = dp(8) })
        ...
        // Retain remaining controls in an invisible tray so state & bindings remain active
        val hiddenTray = LinearLayout(this@MainActivity).apply {
            orientation = LinearLayout.VERTICAL
            visibility = View.GONE
        }
        hiddenTray.addView(connectionDetail)
        ...
        hiddenTray.addView(chipLine)
        hiddenTray.addView(transportRail, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            transportRailHeight,
        ))
        hiddenTray.addView(chainCard, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            dp(56),
        ))
        hiddenTray.addView(smartSplitCard, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            dp(56),
        ))
        hiddenTray.addView(mimCard, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            dp(56),
        ))
        hiddenTray.addView(footerWave, LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            dp(44),
    ...
    private fun faceFor(code: String): Typeface = when (code) {
        ...
        "zh" -> Typeface.create("sans-serif-medium", Typeface.NORMAL)
    ...
    private fun showConnectionProgress(title: String, detail: String) {
        ...
        if (autoScanIndex >= 0) {
            ...
            connectionTitle.text = title.uppercase()
            connectionDetail.text = detail
        }
        tapPromptText.text = Strings.t("Connecting…")
    ...
    private fun showConnected(restored: Boolean = false) {
        ...
        tapPromptText.text = Strings.t("Tap to disconnect")
    ...
    private fun showDegraded() {
        ...
        connectionTitle.setTextColor(AMBER_TEXT)
        connectionTitle.text = Strings.t("DEGRADED")
        tapPromptText.text = Strings.t("Tap to disconnect")
    ...
    private fun showFailure(detail: String? = null) {
        ...
        connectionTitle.text = Strings.t("FAILED")
        tapPromptText.text = Strings.t("Tap to retry")
    ...
    private fun showDisconnected(detail: String = Strings.t("Tap the dial to connect")) {
        ...
        stopSessionTimer()
        resetMetrics()
        connectionTitle.setTextColor(android.graphics.Color.parseColor("#64748B"))
        connectionTitle.text = Strings.t("DISCONNECTED")
        tapPromptText.text = Strings.t("Tap to connect")
        timerDisplay.text = "00:00:00"
        footerWave.setLit(false)
        connectionDetail.text = detail
        setModeEnabled(true)
