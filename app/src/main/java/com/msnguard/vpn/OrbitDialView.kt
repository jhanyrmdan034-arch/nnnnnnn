...
class OrbitDialView(
    ...
    private fun drawCore(canvas: Canvas, cx: Float, cy: Float, ring: Float, accent: Int, active: Boolean) {
        ...
        // Royal electric blue button fill matching image-1.jpg
        val blueTop = 0xFF3B82F6.toInt()
        val blueCenter = 0xFF2563EB.toInt()
        val blueBottom = 0xFF1D4ED8.toInt()

        // Ambient blue drop shadow / glow
        paint.style = Paint.Style.FILL
        val glowAlpha = when (state) {
            State.CONNECTED -> 0.55f + pulse * 0.15f
            State.CONNECTING -> 0.45f + pulse * 0.10f
            else -> 0.38f
        }
        val shadowColor = Sculpt.withAlpha(0xFF2563EB.toInt(), glowAlpha)
        paint.setShadowLayer(dp(if (active) 26 else 20).toFloat(), 0f, dp(6).toFloat(), shadowColor)
        paint.color = blueCenter
        canvas.drawCircle(cx, cy, r, paint)
        paint.clearShadowLayer()

        // Body gradient lit from top-left
        paint.shader = LinearGradient(
            cx - r * 0.7f, cy - r * 0.7f, cx + r * 0.6f, cy + r * 0.9f,
            intArrayOf(blueTop, blueCenter, blueBottom),
        ...
        // Specular highlight near top-left for glossy modern look
        paint.shader = RadialGradient(
            cx - r * 0.25f, cy - r * 0.35f, r * 0.9f,
            intArrayOf(
                Sculpt.withAlpha(Color.WHITE, 0.28f),
        ...
        paint.shader = LinearGradient(
            cx, cy - r, cx, cy + r,
            intArrayOf(
                Sculpt.withAlpha(Color.WHITE, 0.35f),
                Sculpt.withAlpha(Color.WHITE, 0.05f),
            ),
            floatArrayOf(0f, 1f),
            Shader.TileMode.CLAMP,
        )
        ...
        if (active) {
            ...
            paint.color = Sculpt.withAlpha(0xFF60A5FA.toInt(), 0.5f)
    ...
    private fun drawContents(
        ...
        val iconR = r * 0.44f
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = 4.2f * density * geo
        paint.strokeCap = Paint.Cap.ROUND
        paint.color = Color.WHITE

        if (active) {
            paint.setShadowLayer(dp(8) * geo, 0f, 0f, Sculpt.withAlpha(Color.WHITE, 0.6f))
        } else if (state == State.CONNECTING) {
            paint.color = Sculpt.withAlpha(Color.WHITE, 0.85f + pulse * 0.15f)
        }

        // Standby Power Symbol (IEC 5009)
        bounds.set(cx - iconR, cy - iconR, cx + iconR, cy + iconR)
        canvas.drawArc(bounds, -50f, 280f, false, paint)

        // Vertical stem at the top opening
        canvas.drawLine(cx, cy - iconR * 1.15f, cx, cy - iconR * 0.10f, paint)
        paint.clearShadowLayer()
    }
