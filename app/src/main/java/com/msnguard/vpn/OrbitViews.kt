...
class MetricTile(
    ...
    init {
        ...
        val fill = Sculpt.blend(palette.surface, palette.ink, 0.04f)
        background = Sculpt.sculptedRipple(
            resources.displayMetrics.density, fill, 16, accent,
            accent = Sculpt.withAlpha(accent, 0.22f),
        )
        setPadding(context.px(11), context.px(8), context.px(11), context.px(6))
        isClickable = true
        isFocusable = true
        setOnClickListener { onClick() }

        addView(context.orbitLabel(keyText, 8.5f, Sculpt.withAlpha(accentText, 0.95f), medium = true, spacing = 0.12f))

        row = LinearLayout(context).apply {
            orientation = HORIZONTAL
            gravity = Gravity.BOTTOM
        }
        valueView = context.orbitLabel("0", 18f, palette.ink, medium = true, mono = true)
        unitView = context.orbitLabel("B", 9f, Sculpt.withAlpha(palette.faint, 0.95f), medium = true, spacing = 0.06f)
        row.addView(valueView)
        row.addView(unitView, LayoutParams(
            ViewGroup.LayoutParams.WRAP_CONTENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = context.px(2); bottomMargin = context.px(2) })
        ...
        addView(bars, LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            context.px(12),
        ).apply { topMargin = context.px(4); bottomMargin = context.px(4) })
    }
