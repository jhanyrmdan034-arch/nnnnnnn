...
class ExitNodeCard(
    ...
    init {
        ...
        background = Sculpt.sculptedRipple(
            resources.displayMetrics.density, fill, 20,
            rippleColor = palette.neonBlue,
            accent = Sculpt.withAlpha(palette.neonBlue, 0.40f),
        )
        setPadding(px(14), px(12), px(16), px(12))
        isClickable = true
        isFocusable = true
        setOnClickListener {
        ...
        // Flag sits in a sleek circular badge matching image-1.jpg
        flagView = text("\uD83C\uDDFA\uD83C\uDDF8", 22f, palette.ink).apply {
            gravity = Gravity.CENTER
            background = Sculpt.sculptedBackground(
                resources.displayMetrics.density,
                Sculpt.recess(palette.surface, 0.14f),
                22, // circular
                Sculpt.withAlpha(palette.ink, 0.12f),
            )
        }
        addView(flagView, LayoutParams(px(44), px(44)))

        val column = LinearLayout(context).apply { orientation = VERTICAL }
        keyView = text("Auto · WireGuard", 11.5f, Sculpt.withAlpha(palette.faint, 0.95f), medium = true)
        ipView = text("United States", 16f, palette.ink, bold = true).apply {
            setSingleLine(true)
            ellipsize = TextUtils.TruncateAt.END
        }
        locView = text(Strings.t("tap to change"), 10f, Sculpt.withAlpha(palette.faint, 0.7f)).apply {
        ...
        addView(chevronView, LayoutParams(px(20), px(20)))
    }
    ...
    fun render(
        ...
        val resolvedFlag = if (!countryCode.isNullOrBlank()) {
            IpFormatter.flag(countryCode)
        } else {
            "\uD83C\uDDFA\uD83C\uDDF8" // 🇺🇸
        }
        flagView.text = resolvedFlag

        val countryName = when {
            countryCode.equals("US", ignoreCase = true) -> "United States"
            countryCode.equals("DE", ignoreCase = true) -> "Germany"
            countryCode.equals("GB", ignoreCase = true) -> "United Kingdom"
            countryCode.equals("CA", ignoreCase = true) -> "Canada"
            countryCode.equals("NL", ignoreCase = true) -> "Netherlands"
            countryCode.equals("FR", ignoreCase = true) -> "France"
            countryCode.equals("JP", ignoreCase = true) -> "Japan"
            countryCode.equals("SG", ignoreCase = true) -> "Singapore"
            !countryCode.isNullOrBlank() -> PsiphonRegions.name(countryCode) ?: countryCode
            else -> "United States"
        }

        ipView.text = countryName
        keyView.text = if (tunnelled) "Auto · WireGuard" else "Auto · Ottawa"
        contentDescription = "سرور: $countryName"
    }

    fun updateServerInfo(serverName: String, flagEmoji: String, modeName: String = "Auto · WireGuard") {
        ipView.text = serverName
        flagView.text = flagEmoji
        keyView.text = modeName
    }
